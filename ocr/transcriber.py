...
class OCRTranscriber:
    def extract(self, image_bytes) -> Tuple[str, List[OCRTimelineEvent]]:
        ...
        prompt = (
            "Extract timeline from tachograph receipt. Read each row exactly as printed. "
            "Extract 'start_time_raw' (HH:MM), 'duration_raw' (HHhMM), and 'pictogram_raw' "
            "(the exact symbol like *, o, h, X). Stop extraction immediately at '--- Σ ---'."
        )
